...
- Set `enabled` in `src/unit-progression.config.ts` to `true` or `false`, then reload the game. The switch controls the entire system for both allied and enemy melee/ranged minions: kill tracking, stat growth, level-up healing, stars, and size growth. Disabling it restores their original appearance and base stats. Each new unit starts fresh at level 0 without a star; surviving units retain progress across waves, and restarting resets it.
- Reaching levels 1, 2, 3, 4, and 5 requires another 2, 4, 6, 8, and 10 kills respectively (2, 6, 12, 20, and 30 total). Their stars are bronze, green, blue, red, then yellow. Each level adds 8% of the original sprite size, reaching 140% at level 5. Health bars, stars, and projectile origins follow the enlarged unit.
- Each level adds 10% of the level-0 base attack damage and maximum health, without compounding. Level 0 starts at 10 damage / 60 HP; levels 1–5 have 11/66, 12/72, 13/78, 14/84, and 15/90 respectively. Values are rounded to whole numbers. A level-up immediately restores health to the new maximum. Level 5 is the cap; later kills do not trigger additional healing or growth.
- Damage growth applies to melee, ranged attacks, enemy contact damage, and attacks on the player or bus. The bus retains its doubled base damage (20 at level 0, 30 at level 5). Projectiles capture damage at release, so leveling up or dying after firing does not change that shot's damage. Health labels and bar fill use each unit's current maximum health.
- Only the killing blow earns credit, including ranged projectiles. Recycled projectiles clear their previous shooter; dead shooters cannot level up or revive from a delayed hit. Player kills count for enemy minions; the bus is not a unit. A Boss only grants credit on permanent defeat, not when it temporarily vanishes before the last-stand rain. The player and Boss do not gain levels themselves.
- Stars are procedural five-point shapes. No new image assets or animation changes are required. Thresholds, growth, and colors are alongside the master switch in the configuration file.
...
- A projectile aims at the target's position at release and follows a ballistic arc under 650 px/s² gravity. Each shot varies its distance-based flight time (0.65–1.15 s) by ±20%, giving it a higher or lower arc. Both velocity components are recalculated to reach the same target snapshot; the trajectory stays fixed after release, so moving can still dodge it. This also applies to allied arrows and each Boss volley arrow, retaining the Boss speed multiplier. Tune `combatConfig.projectileFlightTimeVariation` to adjust the variation.
